...
SELECT COUNT(*) FROM	 board; --112

-- 예를 들어 현재 보고자 하는 페이지의 번호가 2페이지라면,
-- 게시물의 번호는 11번부터 20번까지의 게시물이 화면에 표시되게 합니다
SELECT * FROM board;
SELECT * FROM board ORDER BY num DESC;
SELECT rownum, * FROM board ORDER BY num DESC; -- rownum 사용하면 * 사용불가
...
-- rownum 과 * 를 모두 사용하기 위해 서브쿼리를 사용
...
-- 게시물 갯수가 많아질수록 위의 명령의 실행속도가 느려집니다. 그를 해결하기 위해 아래의 방법을 사용합니다.
SELECT * FROM (SELECT * FROM (SELECT rownum AS rn, b.* FROM (SELECT * FROM board ORDER BY num DESC) b)
